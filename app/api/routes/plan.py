...
from app.todo.schemas import PlanRequest, ReplanRequest, TaskStatusRequest, TaskTransitionRequest
...
@plan_bp.delete("/api/v1/plan/clear")
def plan_clear():
    """清空某用户的全部学习计划（连同学任务），供前端「清除缓存」使用。"""
    user_id = (request.args.get("user_id", "") or "").strip()
    if not user_id:
        raise APIError(CODE_VALIDATION, "user_id 必填", 422)
    _ensure_db()
    try:
        removed = todo_store.delete_user_plans(_config(), user_id)
    except Exception:  # noqa: BLE001
        current_app.logger.exception("学习计划清空失败")
        raise APIError(CODE_PLAN, "学习计划清空失败", 500)
...
@plan_bp.post("/api/v1/plan/<plan_id>/tasks/<task_id>/status")
def task_set_status(plan_id: str, task_id: str):
    """手动设置任务状态（前端勾选「已掌握」用，可来回切换）。"""
    raw = request.get_json(silent=True)
    if raw is None:
        raise APIError(CODE_JSON_INVALID, "请求体必须为合法 JSON", 400)
    req = _validate(raw, TaskStatusRequest)
    _ensure_db()
    try:
        task = todo_store.set_task_status(_config(), plan_id, task_id, req.status)
    except ValueError as exc:
        raise APIError(CODE_VALIDATION, str(exc), 422)
    except Exception:  # noqa: BLE001
        current_app.logger.exception("任务状态设置失败")
        raise APIError(CODE_PLAN, "任务状态设置失败", 500)
    return ok_response(task.model_dump())
