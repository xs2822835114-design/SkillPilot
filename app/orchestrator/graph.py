  ...
  START → orchestrator_agent（意图识别 + 结构化入参解析）
       └─ 按 intent 条件路由：
            ...
            job_search      → job_requirement_node  ─┤（目标画像）
                              └→ learning_plan_node（LLM 直出结构化学习计划）
                                    → reply_node → END

说明：
- 业务节点为纯函数（app/agents/routing | learning_plan_agent），任何异常不外抛，降级由 reply_node 呈现；
