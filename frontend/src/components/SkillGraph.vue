  ...
  <section class="panel">
    <div class="panel-head">
      ...
      <span class="legend">
        ...
        <i style="background: #1f9d56">已掌握</i>
      </span>
    </div>
    ...
    <svg v-else :viewBox="`0 0 ${box.w} ${box.h}`" class="graph">
      ...
      <g>
        ...
        >
          <title>{{ n.name }}{{ n.mastered ? '（已掌握）' : '' }}</title>
          <circle
            :r="n.name.length > 8 ? 11 : 9"
            :fill="n.mastered ? '#1f9d56' : (colorBy[n.category] || '#888')"
            :fill-opacity="n.mastered ? 1 : 0.9"
            :stroke="n.mastered ? '#168a4a' : '#fff'"
            stroke-width="1.5"
          />
          <text
            y="20"
