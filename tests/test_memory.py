...
def test_tc_m12_routes_validation(app_client):
    resp = app_client.post("/api/v1/memory/remember", data="not json", content_type="application/json")
    assert resp.status_code == 400
    resp = app_client.post(
        "/api/v1/memory/remember",
        json={"user_id": "u", "namespace": "bogus", "key": "k", "text": "t"},
    )
    assert resp.status_code in (422, 503)  # 无 DB 时为 503；有 DB 为非法命名空间 422
